...
struct StoryBrain {
    ...
    mutating func nextStory(_ userChoice: String) {
        // Bu kısımda basitçe o an ki array konumumuzu yönlendiriyoruz,
        // Eğer 1. seçeneği seçersek o sıraya, diğerini seçersek destionationNumber i farklı bir sıraya çekiyoruz.

        let currentStory = stories[destionationNumber]
        if userChoice == currentStory.choiceOne {
            destionationNumber = currentStory.choiceOneDestination
        } else if userChoice == currentStory.choiceTwo {
            destionationNumber = currentStory.choiceTwoDestination
        }
    }

    func getStoryTitle() -> String {
        return stories[destionationNumber].title
    }

    func getChoice1() -> String {
        return stories[destionationNumber].choiceOne
    }

    func getChoice2() -> String {
        return stories[destionationNumber].choiceTwo
    }
}
